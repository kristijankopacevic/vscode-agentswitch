...
describe('buildAuthCommand', () => {
  ...
  it('builds Codex login using the resolved binary path, quoted for a path with spaces, with the PowerShell call operator', () => {
    const cmd = buildAuthCommand('codex', 'login', 'C:\\Program Files\\codex.exe');

    expect(cmd).toBe('& "C:\\Program Files\\codex.exe" login');
  });

  it('builds Codex logout using the resolved binary path, with the PowerShell call operator', () => {
    const cmd = buildAuthCommand('codex', 'logout', 'C:\\tools\\codex.exe');

    expect(cmd).toBe('& "C:\\tools\\codex.exe" logout');
  });

  it('builds Codex status as "login status" (codex has no separate status verb), with the PowerShell call operator', () => {
    const cmd = buildAuthCommand('codex', 'status', 'C:\\tools\\codex.exe');
  ...
  it('throws for codex when no binary path is given, rather than emitting a broken command', () => {
    expect(() => buildAuthCommand('codex', 'login', null)).toThrow();
  });
});
